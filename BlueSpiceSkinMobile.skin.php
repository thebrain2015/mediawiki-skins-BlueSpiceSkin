					...
					<ul id="head-navigation">
						<?php foreach ($this->data['sidebar'] as $bar => $cont) { ?>
							<?php foreach ($cont as $key => $val) { 
								$aText = explode( '|', $val['text']);
								$val['text'] = $aText[0];
								$sBg = '';

								if( !empty($aText[1]) && $oFile = wfFindFile( trim($aText[1]) ) ) {
									...
									if(is_object($oFile) && $oFile->exists()) {
										...
										$sBg = 'style="background:url('.$sUrl.') left no-repeat; padding-left: 30px;"';
									}
								}

								?>
								<li id="<?php echo Sanitizer::escapeId($val['id']) ?>" <?php if ($val['active']) { ?> class="active" <?php }
								?>><a href="<?php echo htmlspecialchars($val['href']) ?>" <?php echo $sBg ?> <?php echo Xml::expandAttributes(Linker::tooltipAndAccesskeyAttribs($val['id'])) ?>><?php echo htmlspecialchars($val['text']) ?></a></li>
								<?php } ?>
							<?php } ?>
					</ul>
