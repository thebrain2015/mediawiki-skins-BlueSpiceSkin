$(document).ready(function() {
	if ($("#bs-nav-small").length != 0 && $("#bs-left-resize-btn").length != 0) {
		...
		$("#bs-left-resize-btn").bind('click', function() {
			if ($('#bs-nav-small').css('display') == 'none') {
				...
				var obj = {width : 170};
				$(document).trigger("BsSkinSetSidebarWidth", [obj]);
				var width = obj.width;
				$("#bs-left-column").animate({width: width + 'px'});

				if ($("#bs-left-column").css('float') == 'right') {
					$("#bs-content-column, #footer").animate({"margin-right": (width + 30) + 'px'});
				} else {
					$("#bs-content-column, #footer").animate({"margin-left": (width + 30) + 'px'});
				}
	...
	if ($("#bs-afterarticlecontent div.bs-readers").length > 0) {
		...
		$("div.bs-authors > fieldset > div").each(function(i, v) {
			authors.append($(v));
		});
		$("div.bs-readers > fieldset > div").each(function(i, v) {
			readers.append($(v));
		});
