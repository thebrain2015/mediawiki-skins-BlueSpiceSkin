 ...
 *
 * Version information
 * $LastChangedDate: 2013-06-18 14:14:58 +0200 (Di, 18 Jun 2013) $
 * $LastChangedBy: swidmann $
 * $Rev: 9786 $
 * $Id: BlueSpice.php 9786 2013-06-18 12:14:58Z swidmann $
 *
 * @file
 * @ingroup Skins
 */
...
class SkinBlueSpiceSkin extends SkinTemplate {
	...
	function setupSkinUserCss( OutputPage $out ){
		...
		$out->addModules( 'ext.bluespice.bluespiceskin' );
	}
...
class BlueSpiceSkinTemplate extends BaseTemplate {
	...
	protected function printContentActionsList() {
		...
		foreach ( $this->data[ 'content_actions' ] as $sKey => $aActionTab ) {
			if( $wgVectorUseIconWatch && in_array($sKey, array('watch', 'unwatch') ) ){
				$aOut[] = '<li id="ca-'.Sanitizer::escapeId( $sKey ).'" class="icon">';
		...
		if ( !empty( $aMoreMenuOut ) ) {
			$aOut[ ] = '<li id="ca-more">';
			$aOut[ ] = '  <img id="ca-more-arrow" src="' . $this->data['stylepath'] . '/' . $this->data['stylename'] . '/bluespice/bs-moremenu-less.png" alt="more menu" />';
	...
	protected function printNavigationSidebar() {
		...
		foreach ( $this->data[ 'sidebar' ] as $bar => $cont ) {
			...
			if ( $cont ) {
				...
				$aOut[ ] = '  <h5>' . $sTitle . '</h5>';
				if( is_string( $cont ) ) { //i.e. Collection extension
					$aOut[ ] = $cont;
				}
	...
	protected function printUserBar() {
		...
		$aAlreadyRendered = array();
		$aPersonalUrlsFilter = array(  'mytalk', 'preferences', 'watchlist','newmessages', 'mycontris', 'userpage', 'logout', 'anonlogin', 'createaccount', 'anontalk', 'anonuserpage' );
		foreach ( $this->data[ 'personal_urls' ] as $sKey => $aItem ) {
			if ( in_array( $sKey, $aPersonalUrlsFilter ) ) continue;

			$sLnkClass = $aItem['active'] ? 'class="active"' : '';
			$sClass = $aItem['class'] ? 'class="pt-personal-url '.$aItem['class'].'"' : 'class="pt-personal-url"';
			$sHref = $aItem['href'] ? $aItem['href'] : '#';
			$sText = $aItem['text'] ? $aItem['text'] : '';

			$aOut[ ] = '<div '.$sClass.' id="pt-'.$sKey.'">';
			$aOut[ ] = '<a href="'.$sHref.'" '.$sLnkClass.'>'.$sText.'</a>';
			$aOut[ ] = '</div>';

			$aAlreadyRendered[] = $sKey;
		}
		...
		if ( $wgUser->isLoggedIn() ) {
			...
			$aOut[ ] = '  <a href="' .htmlspecialchars( SpecialPage::getTitleFor('Userlogout')->getLocalURL( array( 'returnto' => $wgRequest->getVal('title') ) )  ). '" title="' . $sLoginSwitchTooltip . '">';
			$aOut[ ] = '    <img src="' . $this->data['stylepath'] . '/' . $this->data['stylename'] . '/bluespice/bs-icon-loginswitch-on.png" alt="' . $sLoginSwitchTooltip . '" />';
			$aOut[ ] = '  </a>';
			$aOut[ ] = '</div>';
			...
			$aOut[ ] = '  <img src="' . $this->data['stylepath'] . '/' . $this->data['stylename'] . '/bluespice/' . $sButtonUserImage . '" alt="' . $sUserDisplayName . '" />';
			$aOut[ ] = '  <ul id="bs-personal-menu">';

			$aPersonalUrlsFilter = array( 'userpage', 'logout', 'anonlogin' );
			foreach ( $this->data[ 'personal_urls' ] as $sKey => $aItem ) {
				if ( in_array( $sKey, $aPersonalUrlsFilter ) || in_array($sKey, $aAlreadyRendered) )
					continue;
			...
			$aOut[ ] = Linker::link( $wgUser->getUserPage(), $sUserDisplayName);
			$aOut[ ] = '</div>';
		} else {
			...
			$aOut[ ] = '    <img src="' . $this->data['stylepath'] . '/' . $this->data['stylename'] . '/bluespice/bs-icon-loginswitch-off.png" alt="' . $sLoginSwitchTooltip . '" />';
			$aOut[ ] = '  </a>';
			$aOut[ ] = '</div>';
			$aUserBarBeforeLogoutViews = array( );
			wfRunHooks( 'BSBlueSpiceSkinUserBarBeforeLogout', array( &$aUserBarBeforeLogoutViews, $wgUser, $this ) );
	...
	//Copy of BaseTemplate::getFooterLinks
	...
	//Copy of BaseTemplate::getFooterIcons
	...
	public function execute() {
		...
		?>
				<div id="bs-wrapper">
					...
					<div id="bs-application">
						...
						<div id="footer" <?php $this->html( 'userlangattributes' ) ?>>
							<?php $aFooterIcons = $this->getFooterIcons("icononly");
							wfRunHooks( 'BSBlueSpiceSkinBeforePrintFooterIcons', array( &$aFooterIcons, $this, 'icononly' ) );
							...
							wfRunHooks( 'BSBlueSpiceSkinBeforePrintFooterLinks', array( &$aFooterLinks, $this, '' ) );
				...
				<!-- #bs-skyscraper END -->
				<div id="bs-floater-right" class="<?php echo BsConfig::get( 'MW::WidgetBar::SkinWidgetDirection' ); ?>_flyout">
					<!-- Widgets -->
